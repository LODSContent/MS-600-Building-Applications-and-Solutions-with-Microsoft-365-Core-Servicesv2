...
- **What is the Application ID to associate with the SSO Tab?**: *Enter the **Application (Client) ID** for the Azure AD application you registered previously*
- **What is the Application ID URI to associate with the SSO Tab?**: *Enter the **Application ID URI** for the Azure AD application you registered previously*
...
This hook will run when the component loads. If the page is loaded in a Microsoft Teams client, as it's when added in a tab, it calls the `getAuthToken()` method on the Microsoft Teams JavaScript SDK. When successful, it extracts the ID token Microsoft Teams received from Azure AD and provides to the tab to retrieve the currently signed in user's display name. The user's name is then set, using the `setName()` method, to the React state `name` property.
...
This is done by implementing the [OAuth2 On-Behalf-Of (OBO) flow](https://learn.microsoft.com/azure/active-directory/develop/v2-oauth2-on-behalf-of-flow), but this can't be done client-side, rather it must be done server-side.
...
First, locate, and open the **./src/server/server.ts** file.
...
Add the following code before the lines you just found that sets the listening port on the solution. The comments within the code explain the relevant parts
...
Let's update this to save the ID token to the component's `ssoToken` state property.

Locate the following line in the success callback:
...
We want to get the list of teams the user is a member of once we have an access token that can be used to authenticate with Microsoft Graph. Because we have already added that code, we just need to update the `useEffect()` hook that runs when the `msGraphOboToken` state property is updated.
...
> [!TIP]
> If you see an error in the console about the `getJoinedTeams` dependency on the last `useEffect()` hook you added, it's likely because the `getJoinedTeams` is defined after this hook. To fix this, move the `useEffect()` hook you just modified after the `getJoinedTeams` declaration.
